...
Feature: Validate and Test Post Request for Morbidity API with authorization
  ...
  @testPostNewMorbidity
  Scenario: Verify if user is able to add a new record successfully 
    Given User sets POST request with endpoint "/Morbidity/" with valid authorization
    When User sends Post request with new "<MorbidityName>","<MorbidityTestName>","<MorbidityMarkerRef>","<MorbidityTestUnit>" in user input in json format 
  ...
  @testPostExistingMorbidity
  Scenario Outline: Checking if user is able to add a existing record successfully
    Given User set POST request with endpoint "/Morbidity/" with valid authorization
    When  User sends Post request with Existing  "MorbidityName","MorbidityTestName","MorbidityMarkerRef","MorbidityTestUnit" in Request body in json format 
    Then  Status 200 Ok should display with a Message "Error Occured"
   
  @testPostMorbidityDataMissing
  Scenario Outline: Checking if user is able to add a new record successfully
    Given User with endpoint "/Morbidity/" set POST request with valid authorization 
    When  User sends Post request with any two fields in different combination with "<MorbidityName>","<MorbidityTestName>","<MorbidityMarkerRef>","<MorbidityTestUnit"> in Request body in json format  
    Then  Status 200 ok should display with a Message Missing Items OR Invalid Entry.Check on "Missing Field" Names
    
    @testPostMorbidityNoData
  Scenario: Checking if user is able to add a new record successfully with out request body data
    Given User with endpoint "/Morbidity/" sets POST request with valid authorization
    ...
    @testPostMorbidityInvalidAuthorization
  Scenario Outline: Checking if user is able to add a new record successfully with invalid authorization
    Given User set POST request with endpoint "/Morbidity/" with Invalid authorization
